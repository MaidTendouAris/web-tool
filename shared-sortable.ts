(function (global: any) {
  ...
  type SortAxis = "auto" | "horizontal" | "vertical";
  ...
  type SortableOptions = {
    ...
    axis?: SortAxis;
    getDropLabel: () => string;
    getMovedLabel: (position: number) => string;
  ...
  var controllers = new WeakMap<HTMLElement, SortableController>();
  var activeSession: SortSession | null = null;
  var announcer: HTMLElement | null = null;

  function getItems(controller: SortableController): HTMLElement[] {
  ...
  function getOrder(controller: SortableController): string[] {
    return getItems(controller).map(function (item) { return item.dataset.sortId || ""; }).filter(Boolean);
  }

  function ensureAnnouncer() {
    if (announcer && announcer.isConnected) return announcer;
  ...
  function animateReflow(controller: SortableController, mutate: () => void) {
    var items = getItems(controller);
    var positions = new Map<HTMLElement, DOMRect>();
  ...
  function restoreOrder(controller: SortableController, ids: string[]) {
    var byId = new Map<string, HTMLElement>();
    getItems(controller).forEach(function (item) {
      if (item.dataset.sortId) byId.set(item.dataset.sortId, item);
    });
    ids.forEach(function (id) {
      var item = byId.get(id);
      if (item) controller.options.container.appendChild(item);
    });
  }

  function createGhost(item: HTMLElement) {
    var bounds = item.getBoundingClientRect();
    var ghost = item.cloneNode(true) as HTMLElement;
  ...
  function activateSession(session: SortSession) {
  ...
  function positionGhost(session: SortSession, clientX: number, clientY: number) {
  ...
  function columnCount(controller: SortableController) {
    if (controller.options.axis === "vertical") return 1;
    if (controller.options.axis === "horizontal") return Math.max(2, getItems(controller).length);
  ...
  function findTarget(session: SortSession, clientX: number, clientY: number) {
    ...
    var hit = document.elementFromPoint(clientX, clientY) as HTMLElement | null;
    var direct = hit && hit.closest(controller.options.itemSelector) as HTMLElement | null;
    if (direct && direct.parentElement === controller.options.container) {
      return direct === session.item ? null : direct;
    }
    var nearest: HTMLElement | null = null;
  ...
  function previewOrder(controller: SortableController) {
    if (controller.options.onOrderPreview) controller.options.onOrderPreview(getOrder(controller));
  }

  function movePlaceholder(session: SortSession, clientX: number, clientY: number) {
    var controller = session.controller;
  ...
  function releasePointer(session: SortSession) {
    try {
  ...
  function beginSession(event: PointerEvent, controller: SortableController, item: HTMLElement, handle: HTMLElement) {
    if (!event.isPrimary || (event.pointerType === "mouse" && event.button !== 0)) return;
    if (activeSession) finishSession(true);
    event.preventDefault();
    event.stopPropagation();
    var bounds = item.getBoundingClientRect();
    activeSession = {
  ...
  function pointerMove(event: PointerEvent) {
    ...
    if (!session.started && Math.hypot(distanceX, distanceY) < 5) return;
    event.preventDefault();
  ...
  function pointerEnd(event: PointerEvent) {
    ...
    if (activeSession.started) event.preventDefault();
    finishSession(event.type === "pointercancel");
  }

  function moveWithKeyboard(event: KeyboardEvent, controller: SortableController, item: HTMLElement, handle: HTMLElement) {
    var items = getItems(controller);
  ...
  function bind(options: SortableOptions) {
    var existing = controllers.get(options.container);
    if (existing) {
      existing.options = options;
      return;
    }
    var controller: SortableController = {
      options: options,
      pointerDown: function (event) {
        var target = event.target as HTMLElement | null;
        var handle = target && target.closest(".sort-handle") as HTMLElement | null;
        if (!handle || !options.container.contains(handle)) return;
        var item = handle.closest(controller.options.itemSelector) as HTMLElement | null;
        if (!item || item.parentElement !== controller.options.container) return;
