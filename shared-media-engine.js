...
(function (global) {
    ...
    function workerMain() {
        ...
        function flushLogs() {
            if (logQueue.length)
                scope.postMessage({ event: "logs", data: logQueue.splice(0) });
            lastLog = Date.now();
        }
        ...
        function onLog(event) {
            ...
            if (time)
                progress(seconds(time[1]));
            logQueue.push(event);
            if (logQueue.length >= 50 || Date.now() - lastLog >= 150)
                flushLogs();
        }
        ...
        async function dispatch(method, args) {
            if (method === "load") {
                ...
                try {
                    ...
                    core.setProgress(function (event) { progress(Number(event.time) / 1000000); });
                }
                finally {
            ...
            if (method === "write") {
                core.FS.writeFile(args[0], args[1]);
                return;
            }
            if (method === "stat")
                return { size: core.FS.stat(args[0]).size };
            if (method === "blob") {
            ...
            if (method === "probe" || method === "probeFallback") {
                ...
                logger = function (event) {
                    if (method === "probeFallback" || event.type === "stdout")
            ...
            if (method === "exec") {
                ...
                if (limitIndex >= 0)
                    expectedDuration = seconds(command[limitIndex + 1]);
                else if (endIndex >= 0)
                    expectedDuration = seconds(command[endIndex + 1]) - (startIndex >= 0 ? seconds(command[startIndex + 1]) : 0);
    ...
    function create(options) {
        ...
        worker.onmessage = function (event) {
            ...
            if (message.event === "logs") {
                message.data.forEach(function (entry) { options.log(entry.message); });
                return;
            }
            if (message.event === "progress") {
                options.progress(message.data);
                return;
            }
        ...
        function call(method, args = [], transfer = []) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                try {
                    worker.postMessage({ id: id, method: method, args: args }, transfer);
                }
                catch (error) {
                    pending.delete(id);
                    reject(error);
        ...
        return {
            load: function (script, wasm) { return call("load", [script, wasm], wasm instanceof ArrayBuffer ? [wasm] : []); },
            input: function (path, file) { return call("input", [path, file]); },
            reset: function () { return call("reset"); },
            exec: function (args, scale) { return call("exec", [args, scale]); },
            probe: function (args) { return call("probe", [args]); },
            probeFallback: function (path) { return call("probeFallback", [path]); },
            blob: function (path, mime) { return call("blob", [path, mime]); },
            FS: {
                unlink: function (path) { return call("unlink", [path]); },
                writeFile: function (path, data) { return call("write", [path, data]); },
