(function () {
  ...
  function setButtonText(selector, key) {
    var element = $(selector);
    if (!element) return;
    var nodes = Array.from(element.childNodes).reverse() as any[];
  ...
  function updateRangeFill(fillSelector: string, start: number, end: number, max: number) {
  ...
  function getPreviewCurrentTime() {
    ...
    if (!video.src || !Number.isFinite(current)) return null;
    return current;
  }

  function canApplyRange(start: number, end: number, max: number, changed: "start" | "end" | "both", modal: boolean) {
    var minGap = 0.1;
  ...
  function setClipBounds(start: number, end: number, changed: "start" | "end" | "both") {
    var max = getClipRangeMax();
    var minGap = 0.1;
    var nextStart = clampNumber(start, 0, max);
    var nextEnd = clampNumber(end, 0, max);
    if (nextEnd - nextStart < minGap) {
      if (changed === "start") nextStart = Math.max(0, nextEnd - minGap);
      else nextEnd = Math.min(max, nextStart + minGap);
    }
    if (nextEnd - nextStart < minGap) {
      nextStart = 0;
  ...
  function setGifBounds(start: number, end: number, changed: "start" | "end" | "both") {
  ...
  function requestClipBounds(start: number, end: number, changed: "start" | "end" | "both", modal: boolean) {
    var max = getClipRangeMax();
  ...
  function requestGifBounds(start: number, end: number, changed: "start" | "end" | "both", modal: boolean) {
    var max = getGifRangeMax();
    if (!canApplyRange(start, end, max, changed, modal)) {
  ...
  function previewClipRange() {
    ...
    if (!canApplyRange(start, end, max, "both", true)) return;
  ...
  async function handleMetadata() {
    ...
    try {
      ...
      try { parsed = JSON.parse(output); } catch (_parseError) {}
    } catch (error) {
  ...
  async function handleAudio() {
    ...
    if (format === "wav") args.push("-acodec", "pcm_s16le");
    if (format === "aac") args.push("-acodec", "aac", "-b:a", "192k");
  ...
  async function runAction(action) {
    ...
    try {
      if (action === "metadata") await handleMetadata();
      if (action === "audio") await handleAudio();
  ...
  setupUpload($("#singleUpload"), $("#singleFileInput"), function (files) {
    ...
    renderFiles();
    updateVideoPreview();
    if (singleFile) $("#statusLine").textContent = singleFile.name;
  });
  ...
  setupUpload($("#multiUpload"), $("#multiFileInput"), function (files) {
    ...
    renderFiles();
    if (multiFiles.length) $("#statusLine").textContent = multiFiles.length + " " + t("concatTab");
  });
