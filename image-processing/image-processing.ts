(function () {
  ...
  const preferences = (window as any).WebToolsPreferences;
      ...
      function renderStitchList() {
        ...
        stitch.images.forEach(function (item, index) {
          ...
          li.querySelector(".mini-actions").addEventListener("click", function (event) {
            ...
            if (action === "up") moveStitchItem(currentIndex, currentIndex - 1);
            if (action === "down") moveStitchItem(currentIndex, currentIndex + 1);
            if (action === "remove") {
      ...
      document.addEventListener("keydown", function (event) {
        if (event.key === "Escape" && $("#imagePreviewModal").classList.contains("show")) closeImagePreview();
      });
