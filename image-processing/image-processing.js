...
(function () {
    ...
    function setOptions(selector, keys) {
        ...
        Array.from(select.options).forEach(function (option, index) {
            if (keys[index])
                option.textContent = t(keys[index]);
        });
    }
    function applyLanguage(language) {
        ...
        if (!crop.img)
            $("#cropPreview").innerHTML = '<span class="empty">' + t("waitingImage") + "</span>";
        if (stitch.images.length === 0)
            $("#stitchPreview").innerHTML = '<span class="empty">' + t("waitingImage") + "</span>";
        if (!compress.img)
            $("#compressPreview").innerHTML = '<span class="empty">' + t("waitingImage") + "</span>";
        renderStitchList();
        updateCrop();
        updateStitchPreview();
        updateCompress();
    ...
    function updateCrop() {
        ...
        if (crop.canvas)
            crop.canvas.width = crop.canvas.height = 0;
    ...
    function resizeCrop(pos) {
        ...
        if (crop.ratio !== "free") {
            ...
            if ((handle.length === 2) && Math.abs(dx) < Math.abs(dy))
                useWidth = false;
            if (useWidth)
                r.h = r.w / ratio;
            else
        ...
        if (r.y < 0)
            r.y = 0;
        if (r.x + r.w > crop.displayW)
            r.w = crop.displayW - r.x;
        if (r.y + r.h > crop.displayH)
            r.h = crop.displayH - r.y;
        crop.rect = {
            x: Math.round(r.x),
            y: Math.round(r.y),
    ...
    $("#cropOverlay").addEventListener("pointerdown", function (event) {
        ...
        if (!mode && pos.x >= r.x && pos.x <= r.x + r.w && pos.y >= r.y && pos.y <= r.y + r.h)
            mode = "move";
        if (!mode)
            return;
        event.preventDefault();
    ...
    async function exportCrop(reduce = false) {
        ...
        try {
            ...
            $("#cropOutputMeta").textContent = t("encodeError");
        }
        finally {
            canvas.width = canvas.height = 0;
            $("#cropDownload").disabled = !crop.img;
        }
    }
    $("#cropDownload").addEventListener("click", () => { if (crop.img)
        void exportCrop(); });
    ...
    function moveStitchItem(from, to) {
        if (to < 0 || to >= stitch.images.length || from === to)
            return;
    ...
    function updateStitchPreview() {
        ...
        if (stitch.canvas)
            stitch.canvas.width = stitch.canvas.height = 0;
    ...
    async function exportStitch(maxEdge) {
        ...
        try {
            ...
            $("#stitchMeta").textContent = t("encodeError");
        }
        finally {
            if (canvas)
                canvas.width = canvas.height = 0;
            button.disabled = !stitch.images.length;
        }
    }
    ...
    function showSizeGuard(host, w, h, reduce) {
        ...
        button.textContent = t("reduceSize");
    ...
    function updateCompress() {
        ...
        if (suggested.w !== size.w || suggested.h !== size.h) {
            showSizeGuard($("#compressOutputMeta"), size.w, size.h, () => {
                $("#maxWidth").value = suggested.w;
                $("#maxHeight").value = suggested.h;
                updateCompress();
            });
            return;
        }
        ...
        try {
            ...
            }).finally(function () { canvas.width = canvas.height = 0; if (compress.canvas === canvas)
                compress.canvas = null; });
        }
        catch (_) {
            canvas.width = canvas.height = 0;
            compress.canvas = null;
    ...
    setupUpload($("#compressUpload"), $("#compressFile"), function (files) {
        ...
        clearTimeout(compress.timer);
        loadImage(files[0]).then(function (result) {
            if (loadRevision !== compressLoadRevision)
                return;
            ...
            if (loadRevision === compressLoadRevision)
                $("#compressMeta").textContent = window.WebToolsControls.describeError(error);
        });
    });
