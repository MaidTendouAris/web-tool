...
(function (global) {
    ...
    let channel = null;
    try {
        channel = new BroadcastChannel("web-tools-resources");
    }
    catch (_) { }
    function announce() {
    ...
    if (channel)
        channel.onmessage = () => global.dispatchEvent(new Event("web-tools-resources-changed"));
    function transaction(mode, callback) {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = () => {
                if (!request.result.objectStoreNames.contains("resources"))
                    request.result.createObjectStore("resources", { keyPath: "id" });
            };
            ...
            request.onsuccess = () => {
                ...
                try {
                    ...
                    tx.oncomplete = () => { db.close(); if (mode === "readwrite")
                        announce(); resolve(operation.result); };
        ...
        const task = (async () => {
            ...
            try {
                ...
                if (resource.fileName.endsWith(".wasm")) {
                    if (header[0] !== 0 || header[1] !== 97 || header[2] !== 115 || header[3] !== 109)
                        throw new Error("Invalid WASM resource");
                }
                else if (/^\s*</.test(new TextDecoder().decode(header)))
                    throw new Error("Unexpected HTML response");
    ...
    function createCard(host, ids) {
        const resources = ids.map(id => definitions.find(item => item.id === id));
        if (resources.some(item => !item))
            throw new Error("Unknown resource group");
        ...
        const badge = host.querySelector(".wt-resource-badge");
        const summary = host.querySelector(".wt-resource-summary");
        const downloadButton = host.querySelector("[data-download]");
        const importButton = host.querySelector("[data-import]");
        const checkButton = host.querySelector("[data-check]");
        const bar = host.querySelector("progress");
        const message = host.querySelector(".wt-resource-message");
        ...
        downloadButton.onclick = async () => {
            ...
            try {
                for (const item of resources) {
                    ...
                    if (available(await read(item.id))) {
            ...
            try {
                for (const selected of Array.from(input.files)) {
                    ...
                    if (!item || !selected.size)
                        throw new Error("Resource name mismatch");
                    file = selected.name;
                    render();
