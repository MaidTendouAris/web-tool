...
(function (global) {
    ...
    var selectControllers = new WeakMap();
    var selectControllerSet = new Set();
    var numberControllers = new WeakMap();
    var numberControllerSet = new Set();
    var rangeControllers = new WeakMap();
    var rangeControllerSet = new Set();
    var mediaControllers = new WeakMap();
    var mediaControllerSet = new Set();
    var openSelect = null;
    var selectId = 0;
    var refreshFrame = 0;
    function locale() {
    ...
    function replaceLabel(template, label) {
    ...
    function collect(root, selector) {
        ...
        if (root instanceof Element && root.matches(selector))
            result.push(root);
        return result.concat(Array.from(root.querySelectorAll(selector)));
    }
    function copyControlDimensions(source, target) {
    ...
    function positionSelectMenu(controller) {
        ...
        controller.menu.style.left = Math.max(viewportPadding, Math.min(window.innerWidth - rect.width - viewportPadding, rect.left)) + "px";
    ...
    function chooseOption(controller, index) {
    ...
    function renderSelectMenu(controller) {
    ...
    function syncSelect(controller) {
    ...
    function closeSelectMenu(controller, restoreFocus) {
        if (!controller.open)
            return;
        ...
        if (controller.typeaheadTimer)
            window.clearTimeout(controller.typeaheadTimer);
        controller.typeaheadTimer = null;
        if (openSelect === controller)
            openSelect = null;
        if (restoreFocus && controller.button.isConnected)
            controller.button.focus({ preventScroll: true });
    }
    function openSelectMenu(controller) {
    ...
    function moveSelectActive(controller, offset) {
        if (!controller.open)
            openSelectMenu(controller);
        var options = Array.from(controller.select.options);
        if (!options.length)
            return;
    ...
    function typeaheadSelect(controller, character) {
        if (controller.typeaheadTimer)
            window.clearTimeout(controller.typeaheadTimer);
    ...
    function enhanceSelect(select) {
        if (selectControllers.has(select) || select.multiple || select.dataset.nativeControl === "true")
            return;
        ...
        button.addEventListener("click", function (event) {
            ...
            if (controller.open)
                closeSelectMenu(controller, false);
            else
                openSelectMenu(controller);
        });
    ...
    function syncNumber(controller) {
    ...
    function stepNumber(controller, direction) {
        var input = controller.input;
    ...
    function enhanceNumber(input) {
        if (numberControllers.has(input) || input.dataset.nativeControl === "true")
            return;
    ...
    function enhanceRange(input) {
        if (rangeControllers.has(input) || input.dataset.nativeControl === "true")
            return;
        input.classList.add("wt-range-control");
        var observer = new MutationObserver(function () { syncRange(controller); });
        var controller = { input: input, observer: observer };
    ...
    function enhanceCheckbox(input) {
        if (input.dataset.nativeControl === "true")
            return;
    ...
    function syncMediaText(controller) {
        ...
        if (controller.fullscreenButton) {
            setTooltip(controller.fullscreenButton, document.fullscreenElement === controller.root ? text("exitFullscreen") : text("fullscreen"));
        }
    }
    function syncMedia(controller) {
        var media = controller.media;
        ...
        if (!controller.seeking)
            controller.seek.value = String(Math.round(fraction * 1000));
    ...
    function toggleMedia(controller) {
        var media = controller.media;
    ...
    function toggleFullscreen(controller) {
        ...
        var video = controller.media;
        if (video.webkitEnterFullscreen)
            video.webkitEnterFullscreen();
    }
    function mediaIconButton(action) {
    ...
    function enhanceMedia(media) {
        if (mediaControllers.has(media) || media.dataset.nativeControl === "true")
            return;
        ...
        if (isVideo)
            media.addEventListener("click", function () { toggleMedia(controller); });
        [
            "durationchange",
            "emptied",
            "ended",
    ...
    function pruneDisconnected() {
        selectControllerSet.forEach(function (controller) {
            if (controller.select.isConnected)
                return;
            if (openSelect === controller)
                openSelect = null;
        ...
        numberControllerSet.forEach(function (controller) {
            if (controller.input.isConnected)
                return;
            controller.observer.disconnect();
        ...
        mediaControllerSet.forEach(function (controller) {
            if (!controller.media.isConnected)
                mediaControllerSet.delete(controller);
        });
    }
    function enhance(root) {
        collect(root, "select").forEach(enhanceSelect);
        collect(root, 'input[type="number"]').forEach(enhanceNumber);
        collect(root, 'input[type="range"]').forEach(enhanceRange);
        collect(root, 'input[type="checkbox"]').forEach(enhanceCheckbox);
        collect(root, 'input[type="color"]').forEach(enhanceColor);
        collect(root, "audio, video").forEach(enhanceMedia);
    }
    function refreshAll() {
        pruneDisconnected();
    ...
    function initialize() {
        ...
        document.addEventListener("pointerdown", function (event) {
            ...
            if (openSelect.root.contains(target) || openSelect.menu.contains(target))
                return;
            closeSelectMenu(openSelect, false);
        });
    ...
    function createProgress(host, onCancel) {
        ...
        var label = host.querySelector('[role="status"]');
        var cancel = host.querySelector("button");
        var track = host.querySelector('[role="progressbar"]');
        var fill = host.querySelector(".wt-progress-fill");
        var detail = host.querySelector(".wt-progress-detail");
        ...
        function render() {
            ...
            if (file)
                parts.push(file);
            if (ratio !== null)
                parts.push((stage === "done" ? "" : w.estimate + " ") + Math.round(ratio * 100) + "%");
            else if (stage === "processing")
                parts.push(w.unknown);
        ...
        return {
            ...
            file: function (name, current) { file = name; index = current; ratio = null; render(); },
            stage: function (next) { if (!active)
                return; stage = next; ratio = null; render(); },
            update: function (value) {
                if (!active || stage !== "processing")
                    return;
                if (value !== null && Number.isFinite(value))
                    ratio = Math.max(ratio || 0, Math.min(0.99, Math.max(0, value)));
                render();
            },
            finish: function (state) {
                active = false;
                stage = state;
                ratio = state === "done" ? 1 : ratio;
                clearInterval(timer);
                render();
            }
        };
    ...
    global.WebToolsControls = {
        ...
        refreshRange: function (input) {
            var controller = rangeControllers.get(input);
            if (controller)
                syncRange(controller);
        },
        getMediaController: function (media) {
