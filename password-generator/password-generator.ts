(function () {
  ...
  function t(key: keyof typeof TEXT["en"]): string {
  ...
  function setText(selector: string, key: keyof typeof TEXT["en"]): void {
  ...
  function applyLanguage(language: SupportedLanguage): void {
    ...
    $$<HTMLButtonElement>(".language button[data-lang]").forEach((button) => {
      button.classList.toggle("active", button.dataset.lang === language);
    });
    const spaceOption = $("#separatorInput option[value=' ']") as HTMLOptionElement;
    if (spaceOption) spaceOption.textContent = t("space");
    const noSeparatorOption = $("#separatorInput option[value='']") as HTMLOptionElement;
    if (noSeparatorOption) noSeparatorOption.textContent = t("noSeparator");
  ...
  function randomFrom(value: string): string {
  ...
  function removeSimilar(value: string): string {
    return value.split("").filter((char) => !SIMILAR.includes(char)).join("");
  }

  function getPasswordSets(): string[] {
    const excludeSimilar = ($("#similarInput") as HTMLInputElement).checked;
    const sets: string[] = [];
    if (($("#upperInput") as HTMLInputElement).checked) sets.push(excludeSimilar ? removeSimilar(UPPER) : UPPER);
    if (($("#lowerInput") as HTMLInputElement).checked) sets.push(excludeSimilar ? removeSimilar(LOWER) : LOWER);
    if (($("#numberInput") as HTMLInputElement).checked) sets.push(excludeSimilar ? removeSimilar(NUMBERS) : NUMBERS);
    if (($("#symbolInput") as HTMLInputElement).checked) sets.push(SYMBOLS);
    return sets.filter(Boolean);
  }

  function generatePassword(): string {
    const length = clampNumber(Number(($("#lengthInput") as HTMLInputElement).value), 4, 128);
    const sets = getPasswordSets();
    if (!sets.length) throw new Error(t("chooseCharset"));
    const all = sets.join("");
  ...
  function generatePassphrase(): string {
    const wordCount = clampNumber(Number(($("#wordCountInput") as HTMLInputElement).value), 3, 8);
    const separator = ($("#separatorInput") as HTMLSelectElement).value;
    const words: string[] = [];
  ...
  function estimateBits(secret: string): number {
    if (mode === "passphrase") {
      const wordCount = clampNumber(Number(($("#wordCountInput") as HTMLInputElement).value), 3, 8);
  ...
  function strengthInfo(bits: number): { label: string; width: number; color: string } {
    if (bits < 36) return { label: t("veryWeak"), width: 18, color: "var(--danger)" };
    if (bits < 60) return { label: t("weak"), width: 38, color: "var(--amber)" };
    if (bits < 80) return { label: t("fair"), width: 58, color: "var(--blue)" };
    if (bits < 110) return { label: t("strong"), width: 78, color: "var(--green)" };
  ...
  function generate(): void {
    try {
      const count = clampNumber(Number(($("#countInput") as HTMLInputElement).value), 1, 20);
      ($("#countInput") as HTMLInputElement).value = String(count);
      results = [];
  ...
  function syncMode(): void {
    $$<HTMLElement>(".password-settings").forEach((element) => { element.hidden = mode !== "password"; });
    $$<HTMLElement>(".passphrase-settings").forEach((element) => { element.hidden = mode !== "passphrase"; });
    ($("#lengthInput") as HTMLInputElement).disabled = mode === "passphrase";
    $$<HTMLButtonElement>("[data-mode]").forEach((button) => {
      button.classList.toggle("active", button.dataset.mode === mode);
    });
  }
  ...
  $("#copyAllButton").addEventListener("click", () => {
    if (results.length) void copyText(results.join("\n"), t("copiedAll"));
  });
  $("#lengthInput").addEventListener("input", () => {
    const value = clampNumber(Number(($("#lengthInput") as HTMLInputElement).value), 4, 128);
    ($("#lengthSlider") as HTMLInputElement).value = String(value);
  });
  $("#lengthSlider").addEventListener("input", () => {
    ($("#lengthInput") as HTMLInputElement).value = ($("#lengthSlider") as HTMLInputElement).value;
  });
  $$<HTMLButtonElement>("[data-mode]").forEach((button) => {
